...
select_statement
    : (WITH formula_specification)? SELECT axis_specification_list? FROM cube_specification (
...
member_specification
    : MEMBER member_name AS (
        (QUOTE value_expression QUOTE | value_expression) COMMA member_property_def_list?
    )
    ;
...
value_expression_primary
    : value_expression_primary0 (
        DOT (unquoted_identifier | quoted_identifier | amp_quoted_identifier | function_)
    )*
    ;
