...
options { tokenVocab=SQLSimplifiedLexer;}

start : select_statement SEMI EOF ;

select_statement : SELECT column_list_or_star FROM from_clause join_clause where_clause group_clause having_clause sort_clause union_list 
| SELECT DISTINCT column_list_or_star FROM from_clause sort_clause ;

union_list :  
| UNION select_statement ;

c_expr : columnref binary_op_columnref_list 
| unary_op c_expr 
...
f_expr : Identifier OPEN_PAREN c_expr CLOSE_PAREN 
| OPEN_PAREN c_expr CLOSE_PAREN 
| EXTRACT OPEN_PAREN YEAR_P FROM c_expr CLOSE_PAREN 
| Identifier OPEN_PAREN CASE WHEN c_expr THEN c_expr ELSE integral_or_null END_P CLOSE_PAREN binary_op_fexpr_list 
| SUBSTRING OPEN_PAREN c_expr FROM Integral FOR Integral CLOSE_PAREN 
...
integral_or_null : Integral 
| NULL_P ;

binary_op_fexpr_list :  
| binary_op_fexpr_list binary_op f_expr ;

string_list : StringConstant 
| StringConstant COMMA string_list ;

integral_list : Integral 
| Integral COMMA integral_list ;

binary_op_columnref_list : binary_op columnref 
| binary_op_columnref_list binary_op columnref ;

where_clause : WHERE c_expr 
|  ;

join_clause : JOIN columnref ON c_expr 
...
group_by_list : group_by_list COMMA columnref 
| Identifier 
| Identifier AS collabel 
| table_ref DOT identifier_or_star 
| table_ref DOT identifier_or_star AS collabel 
...
column_list : column_list COMMA columnref 
| Identifier 
| Identifier AS collabel 
| table_ref DOT identifier_or_star 
| table_ref DOT identifier_or_star AS collabel 
...
sortby_list : sortby COMMA sortby_list 
...
from_list : from_list COMMA table_ref 
| Identifier 
| Identifier AS Identifier 
| f_expr AS Identifier ;

from_clause : OPEN_PAREN select_statement CLOSE_PAREN AS table_ref 
| from_list COMMA table_ref 
| Identifier 
| Identifier AS Identifier 
| f_expr AS Identifier ;

sortby : Identifier USING gt_lt 
| Identifier asc_desc 
| Identifier 
| Identifier AS collabel 
| table_ref DOT identifier_or_star 
